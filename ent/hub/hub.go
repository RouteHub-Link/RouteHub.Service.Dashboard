...
func StatusValidator(s enums.StatusState) error {
	...
	case "STATUS_INACTIVE", "STATUS_ACTIVE":
		return nil
	default:
		return fmt.Errorf("hub: invalid enum value for status field: %q", s)
	}
}
...
func DefaultRedirectionValidator(dr enums.RedirectionChoice) error {
	switch dr.String() {
	case "TIMED", "NOT_AUTO", "DIRECT_HTTP", "CONFIRM", "CUSTOM":
		return nil
	default:
		return fmt.Errorf("hub: invalid enum value for default_redirection field: %q", dr)
	}
}
