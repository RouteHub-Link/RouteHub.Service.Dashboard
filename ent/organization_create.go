...
func (oc *OrganizationCreate) SetSocialMedias(tm types.SocialMedias) *OrganizationCreate {
	oc.mutation.SetSocialMedias(tm)
	return oc
}
...
func (oc *OrganizationCreate) createSpec() (*Organization, *sqlgraph.CreateSpec) {
	...
	if nodes := oc.mutation.HubsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
