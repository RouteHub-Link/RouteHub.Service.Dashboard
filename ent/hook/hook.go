...
// The HubFunc type is an adapter to allow the use of ordinary
// function as Hub mutator.
type HubFunc func(context.Context, *ent.HubMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f HubFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.HubMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.HubMutation", m)
}
